...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
      ...
      if (line.size() > 0) {
        if (&line[0] != "#") {
          ...
          if (!strcmp(tokens[0].c_str(), "treefile")) {
            ...
            if (tokens[1].compare("split") != 0) { splits = false; }
            ...
            if (ancstates.size() > 0) ancstates[0] = "_all_";
            else
            ...
            bayesian = true;
            if (tokens.size() > 1) { numreps = atoi(tokens[1].c_str()); }
     ...
     */
    if (periods.size() < 1) { periods.push_back(10000); }
    ...
    */
      ...
      for (fnit = fixnodewithmrca.begin(); fnit != fixnodewithmrca.end();
        ...
        for (unsigned int k = 0; k < rm.getDists()->size(); k++) {
          ...
          for (unsigned int j = 0; j < dista.size(); j++) {
            if (dista[j] != rm.getDists()->at(k)[j]) isnot = false;
          }
          if (isnot == false) {
        ...
        if (ancstates[0] == "_all_" || ancstates[0] == "_ALL_") {
          ...
          for (int j = 0; j < intrees[i]->getInternalNodeCount(); j++) {
            ...
            if (splits) {
              ...
              map<vector<int>, vector<AncSplit>> ras =
              ...
              tt.summarizeSplits(
                  intrees[i]->getInternalNode(j), ras, areanamemaprev, &rm);
              cout << endl;
            }
            if (states) {
              ...
              tt.summarizeAncState(intrees[i]->getInternalNode(j),
                                   rast,
                                   areanamemaprev,
                                   &rm,
                                   state_json);
            ...
            if (splits) {
              ...
              map<vector<int>, vector<AncSplit>> ras =
                  bgt.calculate_ancsplit_reverse(*mrcanodeint[ancstates[j]],
                                                 marginal);
              tt.summarizeSplits(
                  mrcanodeint[ancstates[j]], ras, areanamemaprev, &rm);
            }
            if (states) {
              ...
              tt.summarizeAncState(
                  mrcanodeint[ancstates[j]], rast, areanamemaprev, &rm, tmp);
            }
          }
        }
        if (splits) {
    ...
    for (unsigned int i = 0; i < intrees.size(); i++) { delete intrees[i]; }
  }
