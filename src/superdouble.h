/*
 * superdouble, a class with long double precision but less subject to overflow
 or underflow
...
namespace std {
template <> struct hash<Superdouble> {
public:
  size_t operator()(const Superdouble &sd) const { return sd.hash(); }
};
} // namespace std
