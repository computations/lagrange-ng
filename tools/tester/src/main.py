...
if __name__ == "__main__":
    ...
    runner = lagrange.lagrange(args.program)
    failed_paths = []
    ...
    with rich.progress.Progress() as progress:
        ...
        for member in tar.getmembers():
            ...
            if member_ft == 'newick' or member_ft == 'phylip' or member_ft ==\
                'config':
                extract_with_path(tar, member, args.prefix)
            if member_ft == 'log' or member_ft == 'bgkey' or member_ft ==\
                'bgstates':
                extract_with_set_name(tar, member, 'expected', args.prefix)
            progress.update(extract_task, advance=1.0)

        with util.directory_guard(args.prefix):
            work_paths = []
